...
describe('useOrderActivityChart', () => {
    ...
    describe('Stale data detection', () => {
        it('should detect stale data correctly', async () => {
            ...
            const { result } = renderHook(
                () => useOrderActivityChart({ staleTimeMs: 1000 }),
                { wrapper: createWrapper() }
            )
    ...
    describe('Error message handling', () => {
        it('should handle authentication errors', async () => {
            ...
            vi.mocked(dashboardApi.getOrderActivityChart).mockRejectedValue(authError)
        ...
        it('should handle permission errors', async () => {
            ...
            vi.mocked(dashboardApi.getOrderActivityChart).mockRejectedValue(permissionError)
        ...
        it('should handle validation errors', async () => {
            const validationError = new DashboardApiError('Invalid days parameter', 400)
            vi.mocked(dashboardApi.getOrderActivityChart).mockRejectedValue(validationError)
            ...
            await waitFor(() => {
                expect(result.current.error).toContain('Ungültige Parameter')
            })
        })

        it('should handle network errors', async () => {
            const networkError = new Error('Network Error')
            vi.mocked(dashboardApi.getOrderActivityChart).mockRejectedValue(networkError)
        ...
        it('should handle timeout errors', async () => {
            ...
            vi.mocked(dashboardApi.getOrderActivityChart).mockRejectedValue(timeoutError)
    ...
    describe('Refresh with days parameter', () => {
        it('should refresh with new days parameter', async () => {
            vi.mocked(dashboardApi.getOrderActivityChart).mockResolvedValue(mockActivityData)

            const { result } = renderHook(() => useOrderActivityChart({ initialDays: 30 }), {
                wrapper: createWrapper()
            })
            ...
            expect(dashboardApi.getOrderActivityChart).toHaveBeenCalledWith(30)

            // Refresh with new days parameter
            vi.mocked(dashboardApi.getOrderActivityChart).mockClear()

            await act(async () => {
                await result.current.refresh(7)
            })

            expect(result.current.days).toBe(7)
            // Query will refetch automatically due to query key change
        })
    })
})
...
describe('useOrderActivityChartWithInterval', () => {
    beforeEach(() => {
        vi.clearAllMocks()
        vi.useFakeTimers()
    })
    ...
    it('should use custom refresh interval', async () => {
        ...
        renderHook(() => useOrderActivityChartWithInterval(7, 500), {
            wrapper: createWrapper()
        })
...
describe('useManualOrderActivityChart', () => {
    beforeEach(() => {
        vi.clearAllMocks()
        vi.useFakeTimers()
    })
    ...
    it('should not auto-refresh', async () => {
        ...
        renderHook(() => useManualOrderActivityChart(14), {
            wrapper: createWrapper()
        })
        ...
        expect(dashboardApi.getOrderActivityChart).toHaveBeenCalledWith(14)
